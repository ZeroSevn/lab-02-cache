// Copyright 2020 <artem.malenko@yandex.ru>
...
// 1* mean pointers 2* - mean array
void const** directOrder(size_t const length) {
...
void const** randomOrder(size_t const length) {
  ...
  for (size_t i = 0; i < length - 16; i = i + 16) {
    ...
    *current = &array[random];
    // ссылка на то что хронить (значене)
...
int main() {
  ...
  // 8388608
  std::vector<size_t> cache{16u,   32u,   64u,  128u,  256u,  512u,
                            1024u, 2048u, 4096, 8192u, 12288u};
  for (size_t i = 0; i < cache.size(); i++) {
    ...
    for (size_t j = 0u; j < 100u; ++j) {
      current = tested;
      while (current) current = static_cast<void const* const*>(*current);
    }
    ...
    for (size_t j = 0u; j < 100u; ++j) {
      ...
      while (current_3)
        current_3 = static_cast<void const* const*>(*current_3);  // warmup
    }
    ...
    auto const duration_3 =
        std::chrono::duration_cast<std::chrono::nanoseconds>(
            (end_time_3 - start_time_3) / 1000u)
            .count();

    // end
    std::cout << "For " << cache[i] << " direct  array= " << duration
              << std::endl;
    std::cout << "For " << cache[i] << " reverse array= " << duration_2
              << std::endl;
    std::cout << "For " << cache[i] << " random  array= " << duration_3
              << std::endl;
